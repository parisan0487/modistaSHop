    ...
    const filteredProducts = products.filter((product) => {
        ...
        const matchesColor =
            selectedColors.length > 0
                ? product.variants?.some((variant) => selectedColors.includes(normalizeColor(variant.color)))
                : true;

        const matchesSize =
            selectedSizes.length > 0 ? product.variants?.some((variant) => selectedSizes.includes(variant.size)) : true;
